...
describe('daySummary (§0.77 ショー言及 ・ 3 段階)', () => {
  ...
  it('警告 1 (風バ) → 開催予定 (風バ可能性) ・ high の時間帯ラベルを併記 (ショー名は出さない)', () => {
    const r = daySummary({
      weather: '晴れ',
      badges: b('風バ', '通常', '通常'),
      highShows: [{ label: 'ナイトショー' }],
    });
    expect(r).toBe('晴れ。ナイトショー 等のショーは開催予定 (風バ可能性)。');
  });

  it('警告 2 (風 + 雨) → 中止の可能性 (要素併記)', () => {
    const r = daySummary({ weather: '雨', badges: b('風バ', '雨バ', '通常'), highShows: [{ label: '昼のパレード' }] });
    expect(r).toBe('雨。昼のパレード 等のショーは中止の可能性 (風・雨)。');
  });
