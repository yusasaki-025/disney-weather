...
- 日ごとの総合スコア (**BEST / GOOD / OK / FAIR / NG**) ＋ 朝 / 昼 / 夜 サブスコア (昼パレード時刻を最重視)
...
総合スコア ＝ 100 - (風減点 ＋ 雨減点 ＋ 熱中症減点 ＋ 寒さ減点 ＋ UV減点) です (§0.52 で 5 段階ラベルに更新)。

- **BEST** 90 以上 : 風 ・ 雨 ・ 暑さすべて快適
- **GOOD** 75 以上 : 風バ 等の軽微な注意はあるが問題なし
- **OK** 60 以上 : 中程度の注意
- **FAIR** 40 以上 : 中止リスクが出てきている
- **NG** 40 未満 : 中止の可能性が高い

加えて **バッジ下限ガード** (§0.16 `applyBadgeGuard`) : バッジが「中止」なら総合スコアを NG (≤ 20) に、「中止リスク」「雨キャン」「熱キャン」なら FAIR (≤ 40) に、「風バ」「雨バ」「熱バ」なら GOOD (≤ 80) に上限を掛けます。逆に警告バッジが 1 つも無ければ最低 GOOD (≥ 75) を保証します (`warnFloor`)。

風 ・ 雨 ・ 熱の算定窓は、その日の priority:high なショー時刻 (季節や月によって変わるため、この README には固定時刻を書きません。実際の時刻は [src/data/schedule/](src/data/schedule/) の当月 JSON、無ければ [src/data/showSchedule.js](src/data/showSchedule.js) の `FALLBACK_SCHEDULE` を参照) の前後 1 時間の **平均値** (§0.13.2、最大値は補助表示)。ナイトパレードは通年演目で参考扱い。
