// ヘルプモーダルの 8 セクション内容 (§0.35)。
// テキストを一箇所に集約し、help.js がタブ UI として描画する。HTML 文字列で持つ
// (リンク ・ 強調を含むため)。値は固定で外部入力を含まないため esc 不要。

export const HELP_SECTIONS = [
  {
    id: 'about',
    label: 'このアプリは?',
    html: `
      <p>舞浜 (東京ディズニーランド・シー) の天気予報を比較して、ショー・パレードの<strong>中止リスクを判定</strong>するツールです。</p>
      <p>複数の予報を横並びで見て、「風・雨・暑さ」の観点から行く日を選ぶのに使います。</p>
    `,
  },
  {
    id: 'score',
    label: 'スコアの見方',
    html: `
      <ul>
        <li><strong>BEST (90 - 100)</strong> : 風・雨・暑さすべて問題なし</li>
        <li><strong>GOOD (75 - 89)</strong> : ほぼ快適・軽微な注意のみ</li>
        <li><strong>OK (60 - 74)</strong> : 行けるが要検討</li>
        <li><strong>FAIR (40 - 59)</strong> : 風バ or 雨バ域・注意</li>
        <li><strong>NG (0 - 39)</strong> : 中止リスク高・別日推奨</li>
      </ul>
      <p class="help-note">旧表記との対応 : ベスト → BEST・微妙 → FAIR・別日 → NG (GOOD は新設)。</p>
      <p>スコアは、季節限定の昼パレード時刻の前後 1 時間の<strong>平均値</strong>で、風・雨・熱を減点して算出します。</p>
      <p>バッジが「中止」級のときは、平均スコアが高くてもスコアに上限を掛けます (バッジとスコアの矛盾を防ぐため)。</p>
    `,
  },
  {
    id: 'badge',
    label: 'バッジ',
    html: `
      <ul>
        <li><strong>風</strong> : 通常 → 風バ → 中止リスク → 中止 (ショー別の風閾値。過去の中止記録に基づく)</li>
        <li><strong>雨</strong> : 通常 → 雨バ → 雨キャン → 中止</li>
        <li><strong>熱 (WBGT)</strong> : 通常 → 暑さ注意 → 熱バ → 熱キャン → 中止</li>
      </ul>
    `,
  },
  {
    id: 'glossary',
    label: '用語集',
    html: `
      <ul>
        <li><strong>風バ</strong> : 風で演出が一部変更される可能性 (風バージョン)</li>
        <li><strong>雨キャン</strong> : 雨を理由とする中止 (公式「雨天により中止」)</li>
        <li><strong>風キャン</strong> : 強風を理由とする中止 (公式「強風により中止」)</li>
        <li><strong>平均風速</strong> : ショー時刻の 1時間平均風速 (sustained)。詳細パネルの「風 ◯m/s」はこの値</li>
        <li><strong>突風 (gust)</strong> : 1時間の最大瞬間風速 (ピーク)。中止判定・過去事例検索のベースで、平均風速より大きいのが通常。<strong>カード上の「突風 ◯m/s」はこの値</strong></li>
        <li><strong>熱 (WBGT)・暑さ指数</strong> : 気温に湿度・日射を加味した熱中症リスク指標。カード・詳細の「熱」の数値は<strong>ショー時刻帯 (主要昼公演の前後 1時間) の値</strong>を表示。25 以上で熱バ・31 以上で熱キャン・33 以上でほぼ中止</li>
        <li><strong>熱バ・熱キャン</strong> : 暑さによる演出変更・熱中症対策の中止</li>
        <li><strong>最低 〜 最高 (レンジ)</strong> : 「この日の概要」に出る風・雨・熱の<strong>1日の振れ幅</strong>。カード上の数値はショー時刻帯のピンポイント値なので、1日の幅はこのレンジで確認できる</li>
        <li><strong>中止</strong> : 雨・風・熱いずれかで公演が行われないこと (本ツールは「キャンセル」でなく「中止」に統一)</li>
        <li><strong>パイロカット</strong> : 強風で花火 (パイロ) 部分のみ省略</li>
        <li><strong>キャングリ</strong> : 公演中止時に代わりに行われるグリーティング</li>
        <li><strong>ディズニープレミアアクセス (DPA)</strong> : 時刻指定の鑑賞・体験枠を購入できる有料サービス。本ツールでは「DPA」と表記</li>
        <li><strong>抽選</strong> : 公式アプリの「エントリー受付」のこと。当落式の鑑賞枠申込</li>
        <li><strong>確定情報</strong> : 公式アプリ / 公式サイトから取得した公式発表の時刻・演目 (本ツールの推測値ではない)</li>
      </ul>
    `,
  },
  {
    id: 'sources',
    label: 'データソース',
    html: `
      <ul>
        <li>気象庁 (政府標準利用規約)</li>
        <li>Open-Meteo (CC BY 4.0)</li>
        <li>環境省 暑さ指数 電子情報提供サービス (政府標準利用規約)</li>
        <li>東京ディズニーリゾート公式 (ショースケジュール・出典明記)</li>
        <li>過去風キャン記録 : TSUBASA のディズニーパークブログ + X @tdr_syopare_can</li>
      </ul>
      <p>いずれも出典を明記した個人利用です。</p>
    `,
  },
  {
    id: 'frequency',
    label: '取得頻度',
    html: `
      <ul>
        <li><strong>天気</strong> : リロードで最新 (約 10 分キャッシュ)</li>
        <li><strong>ショースケジュール</strong> : 月 1 回 手動更新</li>
        <li><strong>WBGT</strong> : 4 - 10 月は環境省実値、期間外は気温・湿度からの簡易計算</li>
      </ul>
    `,
  },
  {
    id: 'faq',
    label: 'FAQ',
    html: `
      <dl class="help-faq">
        <dt>Q. 「(要確認)」って何 ?</dt>
        <dd>A. 予測の信頼度が低いサインです。理由は次のいずれか : 過去同条件の中止記録が少ない (3件未満) / 6日以降で予報誤差が大きい / 単一ソースだけ極端な値 / 当日朝の発表で覆る可能性。表示をタップすると詳しい理由が出ます。</dd>
        <dt>Q. 「熱」の数値って何 ?</dt>
        <dd>A. WBGT (湿球黒球温度)・通称「暑さ指数」です。気温そのものではなく、湿度・日射を加味した体感に近い熱中症リスク指標です。カード・詳細・スコア理由の数値はすべて<strong>ショー時刻帯 (主要昼公演の前後 1時間)</strong> の値に統一しており、本ツールでは 25 以上で熱バ・31 以上で熱キャン・33 以上でほぼ中止と判定します (一般には 28 以上で警戒・31 以上で危険)。<strong>1日の振れ幅</strong>は「この日の概要」の「熱 (WBGT) : 18 〜 26」のようなレンジで確認できます。</dd>
        <dt>Q. 気象庁の降水確率が出ない日がある</dt>
        <dd>A. 気象庁の予報は約8日先までです。当日・翌日は時間別、明後日以降は週間予報の降水確率を使っています。それより先 (表の後半) は気象庁の列が「—」になり、スコアは他の予報ソースで算定します。</dd>
        <dt>Q. 文字を大きくしたい</dt>
        <dd>A. ヘッダー右上の「文字サイズ」(小 / 中 / 大) で切り替えられます。「大」は老眼の方向けに思い切って拡大します。選んだサイズは次回も保持されます。</dd>
        <dt>Q. 持ち物に靴やモバイルバッテリーが出ない</dt>
        <dd>A. 天気に依存する持ち物 (傘・上着・日焼け止め等) だけをサジェストしています。歩きやすい靴・モバイルバッテリー・充電ケーブルなど、天候に関わらず必携の常備品は、別途ご準備ください。</dd>
        <dt>Q. 「熱中症警戒級」って何 ?</dt>
        <dd>A. WBGT (暑さ指数) の予測値が 33 以上の日に出すサインです。環境省の「熱中症警戒アラート」と同じ基準を予測値から判定しています (環境省の公式発令そのものではありません)。この日はスコアも自動で「NG」になります。</dd>
        <dt>なぜ今日が「NG」?</dt>
        <dd>風・雨・熱のいずれかで中止リスクが高いと判定されたためです。</dd>
        <dt>公式と公演時刻が違う</dt>
        <dd>スケジュールは月 1 取得のため遅延があります。当日は公式アプリも併用してください。</dd>
        <dt>印刷したい</dt>
        <dd>ブラウザの印刷機能をご利用ください。</dd>
        <dt>同行者と共有したい</dt>
        <dd>この URL を LINE・メッセージで送ると同じ画面を共有できます。</dd>
        <dt>バグ・要望</dt>
        <dd>GitHub の Issues へお寄せください。</dd>
      </dl>
    `,
  },
  {
    id: 'contact',
    label: '連絡先',
    html: `
      <p>ソースコード・Issues : <a href="https://github.com/yusasaki-025/disney-weather" target="_blank" rel="noopener">GitHub (yusasaki-025/disney-weather)</a></p>
      <p class="help-note">本ツールは個人制作で、東京ディズニーリゾート公式とは無関係です。表示は公開予報からの推定であり、当日の運営状況は公式でご確認ください。</p>
    `,
  },
];
