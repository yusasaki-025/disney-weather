...
- **WBGT のソース状態を1つだけ表示** (Yuka さん指摘 ・ 熱バッジセルからの冗長表記排除) :
  - 環境省取得済 : `WBGT 環境省` (緑系ピル)
  - 簡易計算で代替 : `WBGT 簡易計算` (黄色ピル)
  この情報を熱バッジセルからは削除する
...
Yuka さん指摘 : 列ヘッダーが「熱 (WBGT)」と表示されてるのに、セルにも「WBGT」「(推定)」が毎回出てきて冗長 → **セル内は数値とラベルだけ**。「(推定) ・ 簡易計算」はステータスバー (§3.14) で1か所だけ表示。

| WBGT | ラベル | 表示例 |
|---|---|---|
| < 25 | 通常 | `22 通常` |
| 25 ≦ wbgt < 28 | 暑さ注意 | `26 注意` |
| 28 ≦ wbgt < 31 | 熱バ可能性あり | `29 熱バ` |
| 31 ≦ wbgt < 33 | 熱キャン濃厚 | `32 熱キャン` |
| ≧ 33 | ほぼ中止 | `34 中止` |

`wbgtSource` の状態 (`env-jp` or `derived`) はステータスバーで集約表示 (§3.14) :

- 環境省取得済 : ステータスバーに「WBGT 環境省」
- 簡易計算で代替 : ステータスバーに「WBGT 簡易計算」(黄色ピル)

セル単位の「(推定)」表記は廃止。詳細を見たい時はホバー時の `title` 属性で「WBGT 簡易計算による推定値 (環境省データ未取得時)」を補助表示。
...
- 単位 : 風は `9m/s` ・ 雨は `45%` まで併記。**熱 (WBGT) セルは列ヘッダーに「熱 (WBGT)」と書いてあるので「WBGT」プレフィックスは付けない、数値だけ** (例 `29`)
- スマホは単位を省略可
...
| クラス | 用途 | 背景色 |
|---|---|---|
| `badge-normal` | 通常 ・ 雨なし | `#E8F3EA` (薄緑) ＋ 文字 `#2D8F3E` |
