...
#### 実装結果 (2026-06-01 ・ 全 6 項目完了)

- **#1 WBGT 統一** : 仕様の「max 統一」は実装と逆転していた (コード上 `wbgtMax` = ソース日次最大の加重平均が 6/1 で 20、`wbgtShowWindow` = ショー時刻帯平均が 26)。Yuka 判断で **`wbgtShowWindow` (ショー時刻帯) に統一**。バッジ判定 ・ スコア理由が既に ShowWindow ベースで、「26 で熱バ」が数値とバッジが一致して自然なため。カード表示 (`src/ui/table.js`) を `wbgtShowWindow ?? wbgtMax` に変更。**スコア ・ バッジのロジックは不変** (表示のみ統一)。ショー個別 (`showRisk.js`) は元々ショー時刻の平均ベースで同思想のため変更なし。
- **#1b 雨** : カード ・ スコア理由とも既に `popShowWindow ?? popMax` で統一済みのため変更不要 (確認のみ)。
- **#1c ★★ レンジ表示** : `src/score/scoring.js` に `hourlyRange(forecasts, field)` を追加し、metrics に `windRange` / `gustRange` / `popRange` / `precipRange` / `wbgtRange` (`{ min, max } | null`) を公開。`src/ui/table.js`「この日の概要」に「風速 : 1 〜 6 m/s (突風 4 〜 14 m/s)」「雨 : 確率 0 〜 70% / 雨量 0 〜 19.8 mm/h」「熱 (WBGT) : 24 〜 28」の 3 行を追加。**命名メモ** : 仕様の `windMin/windMax` は既存 `windMax` (ソース日次最大の加重平均) と衝突するため、全ソース hourly の最低 〜 最高を示す range オブジェクトに変更した (役割が別軸のため)。
- **#2 用語整理** : `src/ui/help-content.js` 用語集に「熱 (WBGT) ・ 暑さ指数」「最低 〜 最高 (レンジ)」を追加、FAQ「熱の数値って何 ?」を ShowWindow 基準 + 実際の閾値 (25/31/33) + レンジ言及に更新。
- **#3 検証** : Vitest 151 件 pass (`hourlyRange` ・ range フィールド ・ ShowWindow がレンジ内に収まる整合性ガードを追加)。lint ・ build クリーン。dev サーバーを DevTools で実描画確認 (6/1 : カード WBGT 26 = スコア理由 WBGT26、レンジ 熱 24 〜 28 ・ 風速 1 〜 6 ・ 突風 4 〜 14、単位 `.unit` margin-left = 1px で食い込みなし)。
- **#4 単位食い込み** : `src/styles.css` `.cat-val .unit` の `margin-left: -3px` → `1px`。`.vg` グループ化 (§0.56.1) で PC ・ スマホ共通 1px でよいため旧スマホ専用 `0` 上書きを撤廃。
